#!/usr/bin/env python3
"""Lock rebuild: posted batting orders + probable/confirmed SPs only.
TBD teams LAD/TEX/SEA/ATH/SD/SF: top projected hitters allowed (flagged).
Hard rules: max 5 hitters/team, $50k, >=2 games, validate all rows before write.
"""
...
OUT = ROOT / "lineups-50-lock-tonight.csv"
OUT_SUM = ROOT / "lineups-50-lock-tonight-summary.txt"
OUT_FLAGS = ROOT / "lineups-50-lock-tonight-flags.txt"
POSTED = ROOT / "lock-rebuild/posted-lineups.json"
SLOTS = ["P","P","C","1B","2B","3B","SS","OF","OF","OF"]
CAP = 50000
N = 50
MAX_EXP = 20
MAX_HITTERS_PER_TEAM = 5
TBD = {"LAD", "TEX", "SEA", "ATH", "SD", "SF"}
RNG = random.Random(19)

def norm(s: str) -> str:
    s = unicodedata.normalize("NFKD", s)
    s = "".join(c for c in s if not unicodedata.combining(c))
    s = s.lower().replace(".", "").replace("'", "").replace("-", " ").replace(" jr", "").replace(" sr", "")
...
def resolve_eligibles(players):
    meta = json.loads(POSTED.read_text())
    posted = meta["posted"]
    prob_sp = meta["prob_sp"]
    by_team = defaultdict(list)
    for p in players:
        by_team[p["team"]].append(p)
    by_team_name = defaultdict(dict)
    for p in players:
        by_team_name[p["team"]][p["nname"]] = p

    # Pitchers: probable/confirmed SPs only (plus TBD teams' named SP if present)
    sp_ids = set()
    sp_notes = []
    for team, pname in prob_sp.items():
        cands = by_team_name.get(team, {})
        p = cands.get(norm(pname))
        if not p:
            # global fallback same team only
            for q in by_team.get(team, []):
                if q["is_p"] and q["nname"] == norm(pname):
                    p = q
                    break
        if p and p["is_p"]:
            sp_ids.add(p["id"])
            sp_notes.append(f"{team}: {p['name']} ({p['id']})")
        else:
            sp_notes.append(f"{team}: MISSING {pname}")

    # LAD missing from some probable lists — allow top LAD SP from projections if needed
    if "LAD" in TBD and not any(p["team"] == "LAD" and p["id"] in sp_ids for p in players):
        lad_sps = sorted([p for p in by_team.get("LAD", []) if p["is_p"] and "P" in p["elig"]], key=lambda x: -x["proj"])
        if lad_sps:
            sp_ids.add(lad_sps[0]["id"])
            sp_notes.append(f"LAD: FALLBACK top proj SP {lad_sps[0]['name']} ({lad_sps[0]['id']})")
    ...
    for team, plist in by_team.items():
        if team in TBD:
            # top projected hitters (non-P) — keep a deep enough pool
            hs = sorted([p for p in plist if not p["is_p"] and "P" not in p["elig"]], key=lambda x: -x["proj"])
            keep = hs[:14]
            for p in keep:
                hit_ids.add(p["id"])
                tbd_hit_ids.add(p["id"])
            hit_notes.append(f"{team}: TBD top{len(keep)} proj hitters")
            continue
        names = posted.get(team, [])
        matched = 0
        for n in names:
            p = by_team_name[team].get(norm(n))
            if p and not p["is_p"] and "P" not in p["elig"]:
                hit_ids.add(p["id"])
                matched += 1
        hit_notes.append(f"{team}: posted matched {matched}/{len(names)}")

    pitchers = [p for p in players if p["id"] in sp_ids]
    hitters = [p for p in players if p["id"] in hit_ids]
    return pitchers, hitters, tbd_hit_ids, sp_notes, hit_notes
...
def validate(roster, pool_ids, allowed_p, allowed_h):
    ...
    if sal > CAP:
        return False, "salary"
    for p in roster["P"]:
        if p["id"] not in allowed_p or not p["is_p"] or "P" not in p["elig"]:
            return False, "pitcher"
    for s in ("C", "1B", "2B", "3B", "SS"):
        h = roster[s]
        if s not in h["elig"] or h["id"] not in allowed_h or h["id"] not in pool_ids:
            return False, f"slot-{s}"
    for p in roster["OF"]:
        if "OF" not in p["elig"] or p["id"] not in allowed_h:
            return False, "of"
...
def main():
    ...
    pool_ids = set(by_id)
    pitchers, hitters, tbd_hit_ids, sp_notes, hit_notes = resolve_eligibles(players)
    pitchers = sorted([p for p in pitchers if p["proj"] >= 6.0], key=lambda x: -x["proj"])
    hitters = [p for p in hitters if p["proj"] >= 4.0]
    allowed_p = {p["id"] for p in pitchers}
    allowed_h = {p["id"] for p in hitters}
    ...
    strong_teams = sorted(team_hit.keys(), key=lambda t: sum(x["proj"] for x in team_hit[t][:4]), reverse=True)

    pairs = []
    for i, a in enumerate(pitchers):
        for b in pitchers[i+1:]:
            if a["team"] == b["team"]:
                continue
            if not (11000 <= a["sal"] + b["sal"] <= 21000):
                continue
            pairs.append((a, b))
    RNG.shuffle(pairs)
    print(f"SPs={len(pitchers)} pairs={len(pairs)} hitters={len(hitters)} tbd_hitters={len(tbd_hit_ids)}", flush=True)
    if len(pairs) < 5 or len(hitters) < 40:
        raise SystemExit(f"pool too small: pairs={len(pairs)} hitters={len(hitters)}")
    ...
    def score(h, stack, gpp):
        ...
        if h["team"] == stack:
            s += 4.0
        # slight penalty for TBD (prefer confirmed)
        if h["id"] in tbd_hit_ids:
            s -= 0.6
        s -= exp[h["id"]] * 0.9
    ...
    def try_one(pa, pb, stack, stack_n, gpp, min_sal):
        ...
        for h in stack_pool:
            ...
            for slot in ("C", "SS", "3B", "2B", "1B", "OF"):
                if slot == "OF":
                    ...
                    continue
                if slot not in h["elig"] or not can_add(h):
                    continue
        ...
        if any(roster[s] is None for s in ("C", "1B", "2B", "3B", "SS")) or len(roster["OF"]) != 3 or sal > CAP:
            return None
            ...
            for _ in range(6):
                ...
                for slot in ("OF", "1B", "3B", "2B", "SS", "C"):
                    ...
                    for i, cur in enumerate(curs):
                        ...
                        for h in by_slot[slot][:50]:
                            if h["id"] in used or over(h["id"]) or slot not in h["elig"]:
                                continue
        ...
        ok, reason = validate(roster, pool_ids, allowed_p, allowed_h)
        if not ok:
            return None
        return roster, sal
    ...
    attempts = 0
    while len(lineups) < N and attempts < 20000:
        attempts += 1
        ...
        stack = RNG.choice([pa["opp"], pb["opp"]] + strong_teams[:10])
    ...
    for rround in range(1, 12):
        ...
        for _ in range(5000):
            ...
            stack = RNG.choice(strong_teams[:14] + [pa["opp"], pb["opp"]])
    ...
    for i, roster in enumerate(lineups, 1):
        ...
        for h in hitters_of(roster):
            if h["id"] in tbd_hit_ids:
                tbd_usage[f"{h['name']} ({h['team']})"] += 1

    if fail_n:
        raise SystemExit(f"VALIDATION FAILED: {fail_n} {fails[:10]}")
    ...
    for pid, c in exp.most_common():
        p = by_id[pid]
        exp_list.append((p["name"], c, 100.0 * c / N, p["proj"], p["sal"], p["team"]))
    ...
    summary = [
        ...
        f"unique_lineups: {len({key(r) for r in lineups})}",
        f"unique_pitcher_pairs: {len(pair_used)}",
        f"validation_pass: {pass_n}",
        f"validation_fail: {fail_n}",
        f"eligible_SPs: {len(pitchers)}",
        f"eligible_hitters: {len(hitters)}",
        f"tbd_teams: {','.join(sorted(TBD))}",
        f"max_hitters_per_team_enforced: {MAX_HITTERS_PER_TEAM}",
        f"min_games_enforced: 2",
        "",
        "top_15_player_exposures:",
    ]
    for name, c, pct, proj, sal, team in exp_list[:15]:
        flag = " TBD" if any(by_id[pid]["name"]==name and pid in tbd_hit_ids for pid,_ in exp.most_common() if by_id[pid]["name"]==name) else ""
        # simpler flag
        is_tbd = any(p["name"]==name and p["id"] in tbd_hit_ids for p in players)
        summary.append(f"  {name} ({team}): {c}/{N} ({pct:.1f}%) proj={proj} sal={sal}{' [TBD]' if is_tbd else ''}")
    summary.append("")
    if over40:
        summary.append("players_over_40pct_exposure:")
        for name, c, pct, *_ in over40:
    ...
    flag_lines = [
        "LOCK REBUILD FLAGS",
        f"TBD teams (top-proj hitters allowed): {', '.join(sorted(TBD))}",
        "",
        "SP notes:",
        *["  " + x for x in sp_notes],
        "",
        "Hitter pool notes:",
        *["  " + x for x in hit_notes],
        "",
        "TBD hitter usage in final 50:",
    ]
    if tbd_usage:
        for name, c in tbd_usage.most_common():
            flag_lines.append(f"  {name}: {c}/50")
    else:
        flag_lines.append("  (none)")
    OUT_FLAGS.write_text("\n".join(flag_lines) + "\n")

    print("\n".join(summary), flush=True)
    print(f"WROTE {OUT}", flush=True)
    print(f"WROTE {OUT_FLAGS}", flush=True)
