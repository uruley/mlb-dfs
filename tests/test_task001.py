...
class WorkloadTests(unittest.TestCase):
    ...
    def test_expensive_opener_stays_opener(self):
        d = resolve_workload(ev(role="opener", announced_starter=True, salary=9800, dk_eligibility="RP", recent_appearances=[{"role": "opener", "innings": "1.0"}]))
    ...
    def test_bulk_not_listed_probable(self):
        d = resolve_workload(ev(announced_starter=False, role="bulk", recent_appearances=[{"role": "bulk", "innings": "4.1"}, {"role": "bulk", "innings": "4.2"}, {"role": "bulk", "innings": "5.0"}]))
...
def _tiny_slate(scratch=False, unposted=False, extra_game="g1"):
    pool = [
        {"ID": "p1", "Name": "Starter A", "Position": "RP", "TeamAbbrev": "AAA", "Salary": "4000", "Game Info": "AAA@BBB", "game_id": extra_game, "mlb_id": "1"},
        {"ID": "p2", "Name": "Starter B", "Position": "SP", "TeamAbbrev": "BBB", "Salary": "5000", "Game Info": "AAA@BBB", "game_id": "g1", "mlb_id": "2"},
        {"ID": "c1", "Name": "Catch", "Position": "C", "TeamAbbrev": "AAA", "Salary": "3000", "Game Info": "AAA@BBB", "game_id": "g1", "posted": "true", "scratched": "true" if scratch else "false"},
        {"ID": "b1", "Name": "First", "Position": "1B", "TeamAbbrev": "AAA", "Salary": "3000", "Game Info": "AAA@BBB", "game_id": "g1", "posted": "false" if unposted else "true"},
        {"ID": "b2", "Name": "Second", "Position": "2B", "TeamAbbrev": "BBB", "Salary": "3000", "Game Info": "AAA@BBB", "game_id": "g1", "posted": "true"},
        {"ID": "b3", "Name": "Third", "Position": "3B", "TeamAbbrev": "BBB", "Salary": "3000", "Game Info": "AAA@BBB", "game_id": "g1", "posted": "true"},
        {"ID": "b4", "Name": "Short", "Position": "SS", "TeamAbbrev": "CCC", "Salary": "3000", "Game Info": "CCC@DDD", "game_id": "g2", "posted": "true"},
        {"ID": "o1", "Name": "OF1", "Position": "OF", "TeamAbbrev": "CCC", "Salary": "3000", "Game Info": "CCC@DDD", "game_id": "g2", "posted": "true"},
        {"ID": "o2", "Name": "OF2", "Position": "OF", "TeamAbbrev": "CCC", "Salary": "3000", "Game Info": "CCC@DDD", "game_id": "g2", "posted": "true"},
        {"ID": "o3", "Name": "OF3", "Position": "OF", "TeamAbbrev": "DDD", "Salary": "3000", "Game Info": "CCC@DDD", "game_id": "g2", "posted": "true"},
    ]
    projs = [{"dk_id": i, "proj_fp": "15" if i=="p1" else "14" if i=="p2" else "8"} for i in ["p1","p2","c1","b1","b2","b3","b4","o1","o2","o3"]]
...
class BuilderTests(unittest.TestCase):
    def test_unknown_selected_blocks_ready_via_exclude(self):
        ...
        self.assertEqual(build_one_lineup(load_players(pool, projs, evidence))["status"], "blocked")
    ...
    def test_reproducible(self):
        pool, projs, evidence = _tiny_slate()
        a = build_one_lineup(load_players(pool, projs, evidence))
        b = build_one_lineup(load_players(pool, projs, evidence))
...
class FreshnessAndExportTests(unittest.TestCase):
    ...
    def test_entries_numeric_only(self):
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            rows = parse_user_entries(src)
            self.assertEqual(len(rows), 4)
            dest = td/"out.csv"
            self.assertEqual(write_entries_upload(dest, rows, [f"id{i}" for i in range(10)]), 4)
    ...
    def test_failed_build_does_not_clobber_ready(self):
        pool, projs, evidence = _tiny_slate()
        with tempfile.TemporaryDirectory() as td:
            td = Path(td); _write_inputs(td, pool, projs, evidence)
            ...
            cash_main(["--slate-date","2026-09-18","--slate-id","syn-1","--pool",str(td/"pool.csv"),"--projections",str(td/"proj.csv"),"--pitcher-evidence",str(td/"ev.json"),"--out-dir",str(td/"out"),"--decision-time","2026-09-18T18:00:00+00:00"])
            self.assertEqual(ready.read_text(), original)
